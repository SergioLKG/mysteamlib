...
- **Sync**: `src/lib/sync/` — `syncLibrary` (idempotente, resumible via `library_synced_at`; **desde migración `0003` (09/09) auto-reintento**: si un intento de logros falla se anota en `user_games.library_sync_error` y el siguiente cron **lo reintenta automáticamente**; el error se limpia al éxito — arregla de raíz el caso "perfil privado al sincronizar → 403 → nunca se curaba". Sir Brante 1272160 seguirá 403 PERMANENTE: Steam no da stats de juegos nunca jugados), `syncMetadata` (Store+schema+global% + HLTB **disabled**, cached 90d, not_found never blocks; `parseReleaseDate()` sanitiza `release_date` ante texto libre tipo "To be announced"), `runDailySync`/`runMetadataSync`/`refreshGame` (batched, `maxMetadataApps=25` para el cron semanal); cron endpoints `api/cron/sync-library|sync-metadata`; `api/games/[appid]/refresh.ts` (protected); crons in `vercel.json` (03:00 daily, Mon 04:00). Rate limiter in `lib/sync/rateLimiter.ts`. **Desde Fase 3 (09/09): `syncLibrary` TAMBIÉN rellena `user_achievements`** (estado unlock por logro desde `GetPlayerAchievements`, catalog upsert autocurativo) — decisión propietario que revierte el "per-usuario minimalista" de Fase 2; migración de datos existentes vía `scripts/backfill-user-achievements.ts` (forced, idempotente; `--retry-only` = sync no forzado para los marcados con error). El cron diario NO repuebla lo ya sincronizado.
  ...
  - **Frontend (v1 IA 09/09)**: `index.astro` (landing + official "Sign in through Steam" button → `/api/auth/login`, legal disclaimer, h1 *"¿Qué me platino ahora?"*), `dashboard.astro` (**Mi Dashboard**: hub logado con perfil (avatar/persona/visibilidad), stats (juegos/logros/platinados/horas) y tarjetas de acceso a herramientas; `DashLayout.astro` = tema+header+nav compartido), `platino.astro` (**Mi próximo platino**: la feature de `03`, página protegida con la isla), logout POST `/api/auth/logout`. **Tono**: primera persona en áreas logadas, tuteo informal en landing — guía en `../.opencode/docs/06-TONO-Y-VOZ.md`.
...
- **FASE 3 EN CURSO (09/09)** — decisiones propiedad cerradas: pesos `difficulty_score` = logros 1.0 · rareza 0.7 · tiempo 0.5 (`src/lib/db/queries/difficulty.ts`); poblar `user_achievements`; solo dashboard logado. Implementado:
  ...
  - **Pendiente**: deploy prod de estos cambios, revisión visual final del propietario, decisión de pesos con datos reales, y arrancar backlog v1.1 (ver `04-ABIERTO-Y-ROADMAP.md`).